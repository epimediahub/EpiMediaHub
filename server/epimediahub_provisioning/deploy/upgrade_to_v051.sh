...
if [ "$(id -u)" -ne 0 ]; then
  echo "Bitte als root starten: sudo bash upgrade_to_v051.sh" >&2
  exit 1
fi

BRANCH="${EPIMEDIAHUB_UPGRADE_BRANCH:-raspberry-v0.5.1-domain}"
...
BACKUP_DIR="/var/backups/epimediahub/v051-$STAMP"
TMP="$(mktemp -d)"
ROLLBACK_ARMED=0
...
grep -q '"api_version":"0.5.1"' "$TMP/health.json" || grep -q '"api_version": "0.5.1"' "$TMP/health.json"
...
if missing: raise SystemExit("Fehlende Tabellen: "+", ".join(sorted(missing)))
...
for col in ("config_version",):
    if col not in cc: raise SystemExit("Fehlende customer-Spalte: "+col)
for col in ("last_seen_at","applied_config_version","last_sync_at","last_sync_status","last_sync_error","sync_bootstrap_hash"):
    if col not in dc: raise SystemExit("Fehlende device-Spalte: "+col)
...
echo
echo "=== EpiMediaHub Raspberry Upgrade erfolgreich ==="
cat "$TMP/health.json"
echo
echo "Backup: $BACKUP_DIR"
echo "Cloudflare und Tailscale wurden nicht veraendert."
