...
def require_admin_api():
    ...
    if not ADMIN_TOKEN or not secrets.compare_digest(request.headers.get("Authorization", ""), expected): abort(401)
...
def login():
    ...
    if request.method == "POST":
        ...
        if ADMIN_PASSWORD and secrets.compare_digest(supplied, ADMIN_PASSWORD):
            session["admin"] = True
...
def dashboard():
    ...
    with db() as con:
        ...
        devices = con.execute("SELECT d.*,c.name customer_name FROM devices d JOIN customers c ON c.id=d.customer_id ORDER BY d.id DESC").fetchall()
...
def web_create_customer():
    ...
    if name:
        with db() as con:
            con.execute("INSERT INTO customers(name,config_json,created_at) VALUES(?,?,?)", (name,json.dumps({"playlist_url":playlist}),iso(utcnow())))
    return redirect(url_for("dashboard"))
...
def web_activation(customer_id):
    guard = web_auth()
    if guard: return guard
    raw_code, token, expires = make_code(), secrets.token_urlsafe(32), utcnow()+timedelta(minutes=60)
    with db() as con:
        customer = con.execute("SELECT * FROM customers WHERE id=? AND enabled=1",(customer_id,)).fetchone()
        if not customer: abort(404)
        con.execute("INSERT INTO activations(customer_id,code_hash,token_hash,expires_at,created_at) VALUES(?,?,?,?,?)",(customer_id,digest(raw_code),digest(token),iso(expires),iso(utcnow())))
    activation_url=f"{PUBLIC_BASE_URL}/connect/{token}"
    return render_template("activation.html", customer=customer, code=display_code(raw_code), activation_url=activation_url, expires_at=iso(expires))
...
def redeem():
    body=request.get_json(silent=True) or {}; code=normalize_code(str(body.get("code",""))); device_id=str(body.get("device_id","")).strip(); platform=str(body.get("platform","android")).strip() or "android"
    if not code or not device_id: return jsonify(error="invalid_request"),400
    now=utcnow()
    with db() as con:
        con.execute("BEGIN IMMEDIATE")
        row=con.execute("SELECT a.*,c.config_json,c.enabled customer_enabled FROM activations a JOIN customers c ON c.id=a.customer_id WHERE a.code_hash=?",(digest(code),)).fetchone()
        if row is None or not row["customer_enabled"]: return jsonify(error="invalid_code"),404
        if row["redeemed_at"] is not None: return jsonify(error="already_used"),409
        if datetime.fromisoformat(row["expires_at"].replace("Z","+00:00")) <= now: return jsonify(error="expired"),410
        session_token=secrets.token_urlsafe(48)
        con.execute("UPDATE activations SET redeemed_at=?,device_id=?,platform=? WHERE id=? AND redeemed_at IS NULL",(iso(now),device_id,platform,row["id"]))
        con.execute("INSERT INTO devices(customer_id,device_id,platform,session_token_hash,created_at) VALUES(?,?,?,?,?) ON CONFLICT(customer_id,device_id) DO UPDATE SET platform=excluded.platform,session_token_hash=excluded.session_token_hash,enabled=1",(row["customer_id"],device_id,platform,digest(session_token),iso(now)))
