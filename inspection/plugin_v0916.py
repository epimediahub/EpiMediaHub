...
HOME_ICONS = [os.path.join(PLUGIN_DIR, "home_%s.png" % name) for name in ("live", "movies", "series", "search", "playlist", "settings")]
...
def prepare_active_theme(theme_id=None):
    ...
    try:
        if os.path.isfile(source):
            shutil.copyfile(source, ACTIVE_THEME_BACKGROUND)
            try:
                os.chmod(ACTIVE_THEME_BACKGROUND, 0o644)
            except Exception:
                pass
        if os.path.isfile(mark_source):
            shutil.copyfile(mark_source, ACTIVE_THEME_MARK)
            try:
                os.chmod(ACTIVE_THEME_MARK, 0o644)
            except Exception:
                pass
...
HOME_SKIN = build_fullscreen_skin(
    ...
    # v0.9.12: one larger centered skin badge replaces the six small tile
...
LIVE_PLAYER_SKIN = '<screen name="EpiLivePlayer" position="0,%d" size="%d,%d" flags="wfNoBorder" backgroundColor="#10151F">' % (
    sy(500), GUI_W, sy(220)
) + (
    '<widget name="picon" position="%s" size="%s" alphatest="on" scale="1" />' % (pos(35, 7), size(92, 48)) +
    label_widget("channel", 145, 10, 540, 42, 27, "left", "center", "#10151F") +
    '<widget name="weather_icon" position="%s" size="%s" alphatest="on" scale="1" />' % (pos(785, 12), size(36, 36)) +
    label_widget("weather", 830, 10, 275, 42, 17, "right", "center", "#10151F", "#BBD4EA") +
    label_widget("clock", 1120, 10, 125, 42, 20, "right", "center", "#10151F", "#D7E7F7") +
    label_widget("now", 35, 58, 1210, 42, 21, "left", "center", "#10151F") +
    label_widget("next", 35, 104, 1210, 35, 18, "left", "center", "#10151F", "#AFC0D4") +
    label_widget("help", 35, 158, 1210, 32, 15, "center", "center", "#10151F", "#7F93A8")
) + '</screen>'
...
def _create_empty_db_with_categories(profile_id, category_sets):
    ...
    try:
        ...
        connection.execute("CREATE TABLE entries (ord INTEGER PRIMARY KEY, kind TEXT NOT NULL, title TEXT NOT NULL, group_name TEXT NOT NULL, logo TEXT, tvg_id TEXT, tvg_name TEXT, url TEXT NOT NULL)")
...
def _replace_group_entries(profile_id, kind, group_name, entries, category_order=0):
    ...
    try:
        ...
        for idx, entry in enumerate(entries):
            rows.append((base_ord + idx + 1, kind, entry.get("title", ""), group_name, entry.get("logo", ""), entry.get("tvg_id", ""), entry.get("tvg_name", ""), entry.get("url", "")))
        if rows:
            ...
            for raw in handle:
                ...
                if len(batch) >= 2500:
                    connection.executemany(
                        "INSERT INTO entries(ord,kind,title,group_name,logo,tvg_id,tvg_name,url) VALUES(?,?,?,?,?,?,?,?)",
                        batch
                    )
                    batch = []
...
def playlist_entries(profile_id, kind=None, group=None, query=None, allowed_groups=None):
    if db_source_mode(profile_id) == "xtream" and sqlite3 is None:
        ...
        sql = "SELECT ord,kind,title,group_name,logo,tvg_id,tvg_name,url FROM entries"
        if where:
    ...
    if sqlite3 is not None and playlist_db_ready(profile_id):
        try:
            ...
            row = connection.execute(
                "SELECT ord,kind,title,group_name,logo,tvg_id,tvg_name,url FROM entries WHERE %s ORDER BY ord LIMIT 1" % where,
                tuple(params)
            ).fetchone()
            connection.close()
            if row:
...
class EpiSplash(Screen):
    ...
    def _nextFrame(self):
        ...
        if not self._paintFrame():
            self.finish()
            return
        # The supplied intro sound is about 3.9 seconds long. Keep the final
    ...
    class EpiMoviePlayer(MoviePlayer):
        ...
        def _handleEpisodeEof(self):
            ...
            try:
                ...
                ref = eServiceReference(4097, 0, next_url)
        ...
        def _switchEpisode(self, delta):
            ...
            try:
                ...
                pass
            ref = eServiceReference(4097, 0, target_url)
...
class EpiLivePlayer(Screen):
    ...
    def __init__(self, session, profile_id, entries, start_index=0):
        ...
        self["actions"] = ActionMap(
            ["OkCancelActions", "DirectionActions", "InfoActions"],
            {
    ...
    def playIndex(self, index):
        ...
        try:
            ...
            pass
        try:
            ref = eServiceReference(4097, 0, entry.get("url", ""))
...
class EpiProfileSelect(Screen):
    ...
    def refreshList(self):
        ...
        if not rows:
            rows = ["%s - GRÜN / GREEN" % tr("no_playlist")]
        self["list"].setList(rows)
        self["info"].setText("Schnellimport: %s   |   M3U-Dateien: %s" % (PLAYLIST_TEXT_FILE, FTP_PLAYLIST_DIR))
...
class EpiMediaHubHome(Screen):
    ...
    def __init__(self, session, profile_id):
        Screen.__init__(self, session)
        self.profile_id = profile_id
        self.tiles = [tr("live"), tr("movies"), tr("series"), tr("search"), tr("switch_playlist"), tr("settings")]
        self.currentIndex = 0
        self["logo"] = Pixmap()
        self["subtitle"] = Label("")
