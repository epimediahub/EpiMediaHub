...
# Country rows: keep flags at x=72..150, move label list to a dedicated column.
...
# Open top-level Mediathek persistently from EpiMediaHubHome. Keep replacement
# on a single source line so it is valid whether the original sits inside a try.
...
t=t.replace(needle,'dialog=self.session.instantiateDialog(EpiMediathekHome); self.session.execDialog(dialog); _mediathek_debug("home_open_mode","persistent_exec")',1)
...
if 'instantiateDialog(EpiMediathekHome)' not in t or 'home_open_mode' not in t:
